...
def t11(X):
    ...
    res = (np.linalg.norm(X, axis=1, keepdims=True).T ** 2) + (np.linalg.norm(X, axis=1, keepdims=True) ** 2) - (2 * (X).dot(X.T))
...
def t12(X, Y):
    ...
    """
    res = ((np.linalg.norm(X, axis=1, keepdims=True).T ** 2) + (np.linalg.norm(Y, axis=1, keepdims=True) ** 2)).T - (2 * X.dot(Y.T))
    return np.sqrt(np.where(res < 0, 0, res))
