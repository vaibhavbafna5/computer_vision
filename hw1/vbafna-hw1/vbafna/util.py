...
def projectOrthographicLines(R, t, L):
    ...
    for i in range(height):
        ...
        p = t_matrix.dot(np.dot(R, L[i, :3]) + t)
        pp = t_matrix.dot(np.dot(R, L[i, 3:]) + t)
...
def projectLines(f, R, t, L):
    ...
    """
    if f == np.inf:
        return projectOrthographicLines(R, t, L)
